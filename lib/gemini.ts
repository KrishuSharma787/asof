...
Rules, all mandatory:
0. The query may be a loose or generic name that several DISTINCT Acts share -- e.g. separate Delhi Amendment Acts to the Court Fees Act passed in different, unrelated years are separate pieces of legislation, not different editions of one Act. Retrieved sources can mix material about more than one of these under the same rough name. Before you use anything from a source, confirm it is actually about the SAME Act as everything else you report: act_name must name ONE specific, precisely-identified Act -- include its year whenever a year is what distinguishes it from a similarly-named Act, rather than echoing the query's own vague wording back if the sources let you be more precise. Every other claim (status_evidence, amendment_timeline, key_judgments) must be about THAT exact Act. If your sources are themselves split across several distinct same-named Acts, report on whichever one they most directly and specifically support, and leave out anything that is actually about a different one of those Acts -- do not blend evidence about two different Acts into a single answer.
